...
func runSurfaceClose(eng *engine.Engine, args []string, wsFlag, dockFlag string, force bool) error {
	...
	dockName, bayName, sName, err := resolveSurfaceArgOrSelf(eng, args[0], wsFlag, dockFlag)
	if err != nil {
		return err
	}
...
func runSurfaceShow(eng *engine.Engine, args []string, wsFlag, dockFlag string) error {
	...
	dockName, bayName, sName, err := resolveSurfaceArgOrSelf(eng, target, wsFlag, dockFlag)
	if err != nil {
		return err
	}
...
func runSurfaceRename(eng *engine.Engine, args []string, wsFlag, dockFlag string) error {
	dockName, bayName, oldName, err := resolveSurfaceArgOrSelf(eng, args[0], wsFlag, dockFlag)
	if err != nil {
		return err
	}
