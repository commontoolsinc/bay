...
// WsNewOptions are options for creating a new bay.
...
// WsNew creates a new bay with surfaces.
...
// wsSkipFunc is called for each bay during batch close. It returns a
// non-empty reason string to skip the bay, or "" to include it.
type wsSkipFunc func(bay *manifest.Bay, dockName string) string

// WsCloseClean closes all clean (non-dirty, no unlanded commits) bays.
...
// WsCloseDone closes bays that are not dirty AND not pending (have an
...
func (e *Engine) BayCloseDone(dockName string, force, dryRun bool, exclude ...string) ([]string, []string, error) {
	return e.wsCloseBatch(dockName, force, dryRun, func(bay *manifest.Bay, dn string) string {
...
// WsUpdate updates bay metadata (branch, PR).
...
// WsDescribe sets (or clears, if desc is "") a bay's description.
...
// WsShow returns detailed information about a bay.
...
// ResolveBay resolves a bay query to (dockName, wsID).
...
// bay names in the same dock. When truncation is needed and ws.Name
...
// matches manifest order. For a new bay (wsID==""), the window goes
