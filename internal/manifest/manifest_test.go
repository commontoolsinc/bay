...
func TestSaveAndLoad(t *testing.T) {
	...
	if len(loaded.Docks[0].Bays) != 1 || loaded.Docks[0].Bays[0].Name != "ws1" {
...
func TestFindDock_ReturnsMutablePointer(t *testing.T) {
	...
	d := m.FindDock("labs")
	d.Bays = append(d.Bays, Bay{Name: "ws1"})
...
func TestResolveBay_ByDockColonID(t *testing.T) {
	...
	if bay.ID != "w1" || dock.Name != "labs" {
		t.Errorf("resolve = ws=%q dock=%q", bay.ID, dock.Name)
	}
}
...
func TestAllBays(t *testing.T) {
	...
	if refs[0].Dock != "labs" || refs[0].Bay.Name != "ws1" {
...
func TestAllBays_ReturnsMutablePointers(t *testing.T) {
	...
	refs := AllBays(m)
	refs[0].Bay.Name = "ws1-renamed"

	if m.Docks[0].Bays[0].Name != "ws1-renamed" {
