...
- **Agent support** — optionally launch AI agents (Claude Code, Codex, Antigravity). Agent surfaces resume on restart and on undo-close via configured `resume_args`. Closing an agent surface prompts for confirmation. Model profiles in config (`[agents.fable] extends = "claude"`) launch the same client pinned to different models, without affecting how resumed sessions pick theirs.
