...
- **Agent support** — optionally launch AI agents (Claude Code, Codex, Antigravity). Agent surfaces resume on restart and on undo-close via configured `resume_args`. Closing an agent surface prompts for confirmation. Built-in model profiles (`fable`, `opus`, `sonnet`, `haiku`) launch Claude pinned to a model — `bay agent opus` just works — and config can add, adjust, or disable profiles, all without affecting how resumed sessions pick their model.
